...
function getNextPageSlugForRedirect() {
  const normalize = (value) => {
    if (!value) return "";
    return value.startsWith("/69d51b49f817587690c6bd5f-preview") ? value : (value.startsWith("/") ? "/69d51b49f817587690c6bd5f-preview" + value : "/69d51b49f817587690c6bd5f-preview/" + value);
  };
...
async function createOrderViaWallet(confirmationToken, paymentMethodId) {
  ...
  const orderData = {
    pageId: "GgDHoaM9pfyWAZjTPk8V8dDZ8JOVXfpr2kbGd4gV5a_qQUxwITygfNZx3pQXoEVx",
    action: "process",
...
const getStates = async (countryIso2Code) => {
  ...
  try {
    if (!countriesStatesCache) {
      const res = await fetch('/69d51b49f817587690c6bd5f-preview/scripts/countriesData/countriesStates.json', { cache: 'no-store' });
...
if (typeof validateAndSendToKlaviyo === "function") {
  try {
    ...
    var pageReadyPayload = {
      ...
      data: { pageName: "checkout1", pageType: "Checkout" },
    };
